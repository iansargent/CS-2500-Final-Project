...
create_table_super = '''CREATE TABLE superintendents('super_id', 'first_name', 'last_name')'''
cur.execute(create_table_super)
#put table into database
super_file = open('superintendents.csv')
super_contents = csv.reader(super_file)
header = next(super_contents)  ## throw away headers
insert_records_super = "INSERT INTO superintendents('super_id', 'first_name', 'last_name') VALUES(?, ?, ?)"
...
#put table into database
admissions_file = open('admissions.csv')
...
#put table into database
finance_file = open('finances.csv')
...
#put table into database
demographic_file = open('demographics.csv')
